                                                   ...
                                                   BOOL supports_vibration)
{
    ...
    c->device              = device;
    c->active              = TRUE;
    c->supports_vibration  = supports_vibration;
    ...
    if (supports_vibration) {
        INT r = hid_send_output_report(device, init_vibration, sizeof(init_vibration),
                                       STADIA_READ_TIMEOUT);
        stadia_dbg("create: init_vibration hid_send returned %d (err=%u)", r, GetLastError());
    }
    ...
    if (!c->input_thread || (supports_vibration && !c->output_thread)) {
...
{
    if (!c->active || !c->supports_vibration)
        return;
...
void stadia_controller_destroy(struct stadia_controller *c)
{
    ...
    if (InterlockedExchange(&c->destroying, 1) != 0)
        return;
